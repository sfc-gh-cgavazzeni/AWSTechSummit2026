...
CREATE OR REPLACE SEMANTIC VIEW RETAILIQ_SV
  ...
  FACTS (
    ...
    ORDERS.ORDER_QUANTITY AS quantity
      WITH SYNONYMS = ('quantity', 'units', 'items')
  ...
  DIMENSIONS (
    ...
    ORDERS.ORDER_MONTH AS DATE_TRUNC('month', order_date)
      ...
      COMMENT = 'Calendar month in which the order was placed, truncated to the first day of the month.',

    ORDERS.ORDER_QUARTER AS DATE_TRUNC('quarter', order_date)
      WITH SYNONYMS = ('quarter', 'quarterly', 'trimestre')
      COMMENT = 'Calendar quarter in which the order was placed, truncated to the first day of the quarter.',
    ...
    CUSTOMERS.CUSTOMER_CITY AS city
      WITH SYNONYMS = ('city', 'città')
      COMMENT = 'City where the customer is located.',

    CUSTOMERS.LOYALTY_TIER AS loyalty_tier
      WITH SYNONYMS = ('loyalty', 'tier', 'membership level', 'card type', 'livello fedeltà')
    ...
    CUSTOMERS.CUSTOMER_AGE_GROUP AS age_group
      WITH SYNONYMS = ('age', 'demographic', 'fascia d''età', 'age group')
  ...
  METRICS (
    ...
    ORDERS.ORDER_COUNT AS COUNT(DISTINCT CASE WHEN status IN ('Completed', 'Returned') THEN order_id END)
      WITH SYNONYMS = ('number of orders', 'orders', '# orders', 'ordini')
    ...
    ORDERS.TOTAL_UNITS_SOLD AS SUM(CASE WHEN status = 'Completed' THEN quantity ELSE 0 END)
      WITH SYNONYMS = ('units', 'items sold', 'pezzi venduti')
    ...
    ORDERS.AVERAGE_DISCOUNT AS AVG(CASE WHEN discount_pct > 0 THEN discount_pct END)
      WITH SYNONYMS = ('discount', 'sconto medio')
    ...
    ORDERS.AVG_SHIPPING_DAYS AS AVG(CASE WHEN channel != 'In-Store' THEN shipping_days END)
      WITH SYNONYMS = ('delivery time', 'shipping speed', 'tempo consegna')
  ...
  -- ════════════════════════════════════════════════════════════════════════════════
  -- VERIFIED QUERIES (pre-validated SQL for top business questions)
  -- ════════════════════════════════════════════════════════════════════════════════
  AI_VERIFIED_QUERIES (
...
ORDER BY 2 DESC'
    ...
    PREMIUM_VS_STANDARD_PRODUCT_MARGIN AS (
      ...
      SQL 'SELECT CASE WHEN p.is_premium = TRUE THEN ''Premium'' ELSE ''Standard'' END AS product_tier,
       SUM(o.total_amount) AS revenue,
